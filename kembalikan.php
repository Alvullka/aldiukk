...
if (isset($_GET['kembalikan'])) {
    ...
    $result = mysqli_query($koneksi, "SELECT no_buku FROM peminjam WHERE id = $id AND tanggal_kembali IS NULL");
    ...
    if ($pinjam) {
        ...
        mysqli_query($koneksi, "UPDATE peminjam SET tanggal_kembali = '$tanggal_kembali' WHERE id = $id");

        // Tambah stok
        mysqli_query($koneksi, "UPDATE buku SET stok = stok + 1 WHERE no_buku = $no_buku");

        echo "<script>alert('Buku berhasil dikembalikan'); window.location='back2.php';</script>";
    } else {
...
<body>
    ...
    <table>
        ...
        <?php
        $query = mysqli_query($koneksi, "SELECT p.id, p.nama_peminjam, p.tanggal_pinjam, k.judul_buku
            FROM peminjam p
